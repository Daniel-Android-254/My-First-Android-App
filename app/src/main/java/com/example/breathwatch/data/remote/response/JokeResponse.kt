...
data class JokeResponse(
    ...
    @Json(name = "flags") val flags: JokeFlags?,
...
data class JokeFlags(
    ...
    @Json(name = "religious") val religious: Boolean,
    @Json(name = "political") val political: Boolean,
    @Json(name = "racist") val racist: Boolean,
    @Json(name = "sexist") val sexist: Boolean,
    @Json(name = "explicit") val explicit: Boolean
)

// Extension function to get the complete joke text
fun JokeResponse.getJokeText(): String {
    return when (type) {
        "single" -> joke ?: "No joke available"
        "twopart" -> "${setup ?: ""}\n\n${delivery ?: ""}"
        else -> joke ?: "No joke available"
    }
}
